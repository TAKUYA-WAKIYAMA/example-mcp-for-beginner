...
for (int i = 0; i < response.ToolCalls.Count; i++)
{
    ...
    var textBlock = result.Content.FirstOrDefault();
    if (textBlock != null)
    {
        Console.WriteLine("=== TextContentBlock のプロパティ一覧 ===");

        foreach (var prop in textBlock.GetType().GetProperties())
        {
            var value = prop.GetValue(textBlock);
    ...
    {
        Console.WriteLine("TextContentBlock が見つかりませんでした。");
    }

    var textblock = result.Content.FirstOrDefault();
    if (textblock is ModelContextProtocol.Protocol.TextContentBlock tcb)
    {
        Console.WriteLine(tcb.Text);
    }

}

// 6. Print the generic response
if (content != null) Console.WriteLine($"Assistant response: {content}");
